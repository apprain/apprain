...
class appRain_Base_Modules_CategorySet extends appRain_Base_Objects
{
    ...
    public function callCategorySetByFiled($method = NULL, $params = NULL)
    {
        ...
        $params[0] = isset($params[0]) ? $params[0] : "1";
        if (($field = str_replace("findby", "", strtolower($method))) != strtolower($method)) {
			...
			return App::Model('Category')->findAll("(type = '{$_type}') AND {$this->fixAditionalCondition($params[0])}");
        }
        else if (($field = str_replace("find", "", strtolower($method))) != strtolower($method)) {
            return App::Model('Category')->find("type = '{$_type}' AND {$this->fixAditionalCondition($params[0])}");
         }
    }
     ...
     */
    public function get_category_recursion($type = NULL, $pid = NULl, $padding1 = '', $padding2 = '')
    {
        // Query data from DB
    ...
    public function deleteCategoryImage($ref = "")
    {
        ...
        if ($ref != "") {
            ...
            if (file_exists($path)) App::Helper('Utility')->deleteFile($path);
        }
    }
        ...
        if (isset($id)) {
            ...
            if ($this->getCategorysettype()) {
                ...
                if (!empty($categories['data'])) {
                    foreach ($categories['data'] as $category) $this->deleteCategoryById($category['id']);
                }
            }
        }
    }
