...
(function (a) {
    a.widget("ui.menu", {_create:function () {
        ...
        b.children("a").addClass("ui-corner-all").attr("tabindex", -1).click(
            function () {
...
/**
 * Updated by Reazaul Karim
 * info@apprain.com
 * www.apprain.org
 *
 */

var __defFieldNameCollection = [];
var __defFieldValueCollection = [];
jQuery(document).ready(function () {
    jQuery('input[type=password],input[type=text],textarea').each(function (k, v) {
        if (jQuery(v).val() != "" && jQuery(v).attr('name') != '') {
            __defFieldValueCollection[(k - 1)] = jQuery(v).val();
            __defFieldNameCollection[(k - 1)] = jQuery(v).attr('name');
        }
    });
});
...
(function (jQuery) {
    jQuery.fn.defaultValue = function () {
        var __value = "";
        for (var c = 0; c < __defFieldNameCollection.length; c++)
            if (__defFieldNameCollection[c] == this.attr('name')) {
                __value = __defFieldValueCollection[c];
                break;
            }
        return __value;
    }
